...
export type SecurePlayerProps = {
  source: string; poster?: string; title?: string; expiresAt?: number;
...
export function SecureVideoPlayer({source,poster,title,onEvent,onRefreshAuthorization}:SecurePlayerProps){
  ...
  return <div className="svp" ref={wrap} onContextMenu={e=>e.preventDefault()}>
    <video ref={ref} src={src} poster={poster} playsInline preload="metadata" aria-label={title??"เครื่องเล่นวิดีโอ"}
